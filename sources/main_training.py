...
def main(data_dir, dest_dir, num_classes, batch_size, num_epochs, keep_feature_extract, weight):
    ...
    if weight:
        ...
        # If no weights are provided, calculate them based on class frequency (inverse of class frequencies)
        pixel_counts = np.array([10650718, 2874775, 3914805, 3792271, 2752034, 2070703, 1908520, 1619572, 1532725, 1567943])  # Example pixel counts
        total_pixels = pixel_counts.sum()
        class_weights = total_pixels / (len(pixel_counts) * pixel_counts)  # Inverse of class frequency
        weight_tensor = torch.FloatTensor(class_weights).to(device)

    # Setup the loss function
    criterion = nn.CrossEntropyLoss(weight=(torch.FloatTensor(weight).to(device) if weight else None), ignore_index=255)
    ...
    print("Save ...")
    torch.save(model_deeplabv3_state_dict, os.path.join(dest_dir, "best_DeepLabV3.pth"))
