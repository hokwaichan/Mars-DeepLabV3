        ...
        for phase in ["train", "val"]:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                if phase == "val":
                    ...
                    label_path = os.path.join(labels_dir, f"{counter:06}_label.png")
                    prediction_path = os.path.join(predictions_dir, f"{counter:06}_pred.png")
                    
                    cv2.imwrite(label_path, cv2.cvtColor(colorized_labels, cv2.COLOR_RGB2BGR))
                    cv2.imwrite(prediction_path, cv2.cvtColor(colorized_preds, cv2.COLOR_RGB2BGR))
